...
def build_from_scorecard(
    ...
    for ak, row in agents.items():
        ...
        for lo, hi in BINS:
            mid = (lo + hi) / 2.0
            scale = acc / max(0.01, mid / 100.0)
            bins[_bin_label(lo, hi)] = {
                "empirical_hit_rate": round(min(1.0, max(0.0, scale * (mid / 100.0))), 4),
                "observations": int(row.get("directional_observations", 0) or 0) // len(BINS),
            }
...
def rebuild_calibration_files() -> Dict[str, Any]:
    ...
    payload = build_from_scorecard(sc, tc)
