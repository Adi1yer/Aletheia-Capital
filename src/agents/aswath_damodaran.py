"""Aswath Damodaran investment agent (hybrid valuation lane)."""

from src.agents.base import BaseAgent
from src.agents.hybrid import HybridAgentMixin


class AswathDamodaranAgent(BaseAgent, HybridAgentMixin):
    hybrid_lane = "valuation"
    hybrid_profile = "dcf_simple"

    def __init__(self, weight: float = 1.0):
        super().__init__(
            name="Aswath Damodaran",
            description="The Dean of Valuation",
            investing_style="Intrinsic valuation, DCF narrative, risk-adjusted returns",
            weight=weight,
            llm_model="ollama-llama",
            llm_provider="ollama",
        )

    def analyze(self, ticker: str, start_date: str, end_date: str, **kwargs):
        return self.run_hybrid_analysis(ticker, start_date, end_date, **kwargs)
